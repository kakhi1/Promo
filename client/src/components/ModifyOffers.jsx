...
function ModifyOffers() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Extract the value property from category, state, and tags fields
    updatedOfferInfo.category = updatedOfferInfo.category.map(
      (item) => item.value
    );
    updatedOfferInfo.state = updatedOfferInfo.state.map((item) => item.value);
    updatedOfferInfo.tags = updatedOfferInfo.tags.map((item) => item.value);

    // Remove numberField if it is empty
    if (!offerInfo.numberField) {
      delete updatedOfferInfo.numberField;
    }

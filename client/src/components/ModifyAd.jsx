...
const ModifyAd = ({ initialAdData }) => {
    ...
    const fetchAdData = async (adId) => {
      try {
        ...
        setFormData({
          ...
          link: adData.link,
          pages: adData.pages,
          endDate: adData.endDate.split("T")[0], // Assuming you want to format endDate for input[type="date"]
          imageUrlDesktop: null, // File objects can't be set from fetched data
        ...
        if (adData.imageUrlDesktop) {
          setDesktopImagePreview(`${imageBaseUrl}${adData.imageUrlDesktop}`);
        }
        if (adData.imageUrlMobile) {
          setMobileImagePreview(`${imageBaseUrl}${adData.imageUrlMobile}`);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append updated fields in a similar manner to AdAdd component
    ...
    try {
      ...
      await axios.patch(`http://localhost:5000/api/ads/ads/${adId}`, data, {
        headers: {

...
const OffersCard = ({
  ...
  const handleCardClick = async () => {
    ...
    try {
      await fetch(`${config.apiBaseUrl}/api/offers/increment-views/${id}`, {
        ...
        body: JSON.stringify({ ip: await fetchUserIP() }),
      });
  ...
  return (
    ...
    >
      {/* Display the numberField on the left side */}
      {numberField > 0 && (
        <div className="absolute top-2 left-4 bg-green-500 rounded-lg text-white text-center font-bold md:text-[36px] text-[30px] w-[40%] h-[20%] z-50 flex  justify-center items-center">
          -{numberField}%
        </div>
      )}
